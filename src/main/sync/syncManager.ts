...
export class SyncManager {
  ...
  private applySites(remoteSites: Site[]): void {
    const config = this.configManager.get();
    const localJson = JSON.stringify(config.sites.map(s => s.id).sort());
    const remoteJson = JSON.stringify(remoteSites.map(s => s.id).sort());

    if (localJson === remoteJson) return; // 差分なし

    log.info(`[Sync] 拠点設定を更新: ${remoteSites.length}拠点`);
    this.configManager.set({ sites: remoteSites });
